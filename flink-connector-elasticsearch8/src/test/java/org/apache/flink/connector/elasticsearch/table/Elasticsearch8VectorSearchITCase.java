...
public class Elasticsearch8VectorSearchITCase {
    ...
    public void testSearchFullTypeVectorTable() throws Exception {
        ...
        IndexResponse response =
                getClient().index(i -> i.index(index).id("1").document(document));
        ...
        String ca = new String(ES_CONTAINER.caCertAsBytes().get());
        ...
        tEnv.executeSql(
                "CREATE TABLE esTable ("
                        ...
                        + String.format(
                                "'%s'='%s'\n",
                                Elasticsearch8ConnectorOptions.SSL_HTTP_CA.key(),
                                ca)
                        + ")");
    ...
    void testSearchUsingFloatArray(String metric) throws Exception {
        ...
        indexSimpleDocument(index, "2", 2L, "Streaming", new float[] {-0.2718f, -0.6527f, -0.7076f});
    ...
    private void createFullTypesIndex(String index) throws IOException {
        ...
        TypeMapping mapping =
                TypeMapping.of(
                        m ->
                                m.properties("id", Property.of(p -> p.long_(LongNumberProperty.of(l -> l))))
                                        .properties("f1", Property.of(p -> p.text(TextProperty.of(t -> t))))
                                        .properties("f2", Property.of(p -> p.boolean_(BooleanProperty.of(b -> b))))
                                        .properties("f3", Property.of(p -> p.byte_(ByteNumberProperty.of(b -> b))))
                                        .properties("f4", Property.of(p -> p.short_(ShortNumberProperty.of(s -> s))))
                                        .properties("f5", Property.of(p -> p.integer(IntegerNumberProperty.of(i -> i))))
                                        .properties("f6", Property.of(p -> p.date(DateProperty.of(d -> d))))
                                        .properties("f7", Property.of(p -> p.text(TextProperty.of(t -> t))))
                                        .properties("f8", Property.of(p -> p.float_(FloatNumberProperty.of(f -> f))))
                                        .properties("f9", Property.of(p -> p.double_(DoubleNumberProperty.of(d -> d))))
                                        .properties("f10", Property.of(p -> p.denseVector(createDenseVectorProperty(2, "cosine"))))
                                        .properties("f11", Property.of(p -> p.denseVector(createDenseVectorProperty(2, "cosine"))))
                                        .properties("f12", Property.of(p -> p.denseVector(createDenseVectorProperty(2, "cosine"))))
                                        .properties("f13", Property.of(p -> p.denseVector(createDenseVectorProperty(2, "cosine")))));
    ...
    private void createSimpleIndex(String index, String similarity) throws IOException {
        ...
        TypeMapping mapping =
                TypeMapping.of(
                        m ->
                                m.properties("id", Property.of(p -> p.long_(LongNumberProperty.of(l -> l))))
                                        .properties("label", Property.of(p -> p.text(TextProperty.of(t -> t))))
                                        .properties("vector", Property.of(p -> p.denseVector(createDenseVectorProperty(3, similarity)))));
    ...
    private void indexSimpleDocument(String index, String docId, Long id, String label, float[] vector)
            throws IOException {
        Map<String, Object> document = new HashMap<>();
        document.put("id", id);
